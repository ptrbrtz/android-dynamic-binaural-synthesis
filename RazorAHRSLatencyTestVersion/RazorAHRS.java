/******************************************************************************************
!!! NOTE: this is a slightly modified version - only used for latency testing !!!
******************************************************************************************/
...
public class RazorAHRS {
	...
	private static final int MSG_ID__YPRT_DATA = 0;
	...
	private static final int MSG_ID__AMGT_DATA = 5;
	...
	private class BluetoothThread extends Thread {
				...
				while (true) {
					...
					if (razorOutputMode == RazorOutputMode.YAW_PITCH_ROLL_ANGLES) {
						if (inBufPos == 12) {	// We received a full frame
							...
							sendToParentThread(MSG_ID__YPRT_DATA, ypr, timestamp);
						...
						if (inBufPos == 36) {	// We received a full frame
							...
							// Forward to parent thread handler
							sendToParentThread(MSG_ID__AMGT_DATA, amg, timestamp);
		 ...
		 * @param msgId
		 * @param data
		 * @param splitLong A long that will be split into two ints for sending
		 */
		private void sendToParentThread(int msgId, Object o, long splitLong) {
			if (callbacksEnabled)
				parentThreadHandler.obtainMessage(msgId, (int) (splitLong >> 32), (int) splitLong, o).sendToTarget();
		}
	...
	private Handler parentThreadHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
				switch (msg.what) {
				case MSG_ID__YPRT_DATA:	// Yaw, pitch, roll and timestamp data
					float[] ypr = (float[]) msg.obj;
					long timestamp = (((long) msg.arg1) << 32) | (msg.arg2 & 0xffffffffL);
					razorListener.onAnglesUpdate(ypr[0], ypr[1], ypr[2], timestamp);
					float3Pool.put(ypr);
					break;
				case MSG_ID__AMGT_DATA:	// Accelerometer, magnetometer, gyroscope and timestamp data
					float[] amg = (float[]) msg.obj;
					timestamp = (((long) msg.arg1) << 32) | (msg.arg2 & 0xffffffffL);
					razorListener.onSensorsUpdate(amg[0], amg[1], amg[2], amg[3], amg[4], amg[5],
							amg[6], amg[7], amg[8], timestamp);
					float9Pool.put(amg);
					break;
